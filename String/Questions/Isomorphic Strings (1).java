...
*/
// 1 HASHMAP (one-way mapping + reverse check)
// O(N)² because hm.containsValue() is O(N)

package String.Questions;
import java.io.*;
import java.util.*;

class IsomorphicStrings1
{
    ...
    static boolean isIsomorphic(String str1, String str2)
    {
        ...
        for (int i = 0; i < str1.length(); i++)
        {
            ...
            //Forward mapping :
            //If HashMap contains the key, its value must match with ch2
            if (hm.containsKey(ch1) && hm.get(ch1) != ch2) {
                return false;
            }

            //Reverse mapping :
            //If ch2 is already mapped to some other key
            if (!hm.containsKey(ch1) && hm.containsValue(ch2)) {
                return false;
            }

            //Add the pair to HashMap
            hm.put(ch1, ch2);
